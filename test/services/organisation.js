...
const OrganisationMocks = require('../../seeds/mock/users')
describe('@services::Organisation', () => {
    ...
    describe('create', () => {
        ...
        it('should work within a transaction', async () => {
            ...
            expect(org.createdByUser).to.be.equal(jenkins.id)
        })
    })

    describe('setCreatedByUser', () => {
        it('should set userId successfully', async () => {
            ...
            expect(orgs).to.have.length(1)
            const org = orgs[0]
            expect(org.createdByUser).to.not.be.equal(user.id)
            await Organisation.setCreatedByUser(org.id, user.id, db)
            ...
            expect(updatedOrg.createdByUser).to.be.equal(user.id)
        })
    })
})
