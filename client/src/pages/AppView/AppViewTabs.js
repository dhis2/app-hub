...
const HeaderSection = ({
    ...
    return (
        <section
            className={classnames(styles.appWrapper, styles.appCardHeader)}
        >
            <AppIcon src={logoSrc} />
            <div>
...
const AppViewStatic = ({ match }) => {
    ...
    const selectedTab = searchParams.get('tab') ?? 'about'

    const compareWithVersion = searchParams.get('compatibleWith')

    console.log(location.search, compareWithVersion)

    const onInstall = (version) => {
        window.top.postMessage({ action: 'install', version }, '*')
    }
        ...
        <Card className={styles.appCard}>
            ...
            <section className={styles.appWrapper}>
                ...
                {selectedTab === 'previous-releases' && (
                    <>
                        <Versions
                            compareWithVersion={compareWithVersion}
...
AppViewStatic.propTypes = {
    match: PropTypes.object.isRequired,
}

export default AppViewStatic
