 ...
 */
const getOrganisationByNameAsync = async (name, knex) => {
    ...
    try {
        const [organisation] = await knex('organisation').select().where('name', name)
        return organisation
    } catch ( err ) {
...
module.exports = getOrganisationByNameAsync
